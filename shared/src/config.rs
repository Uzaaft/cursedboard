...
impl Config {
    ...
    pub fn peer_addrs(&self) -> Vec<SocketAddr> {
        self.network.peers.iter().map(|peer| {
...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ConfigError::InvalidRegex(pattern, e) => write!(f, "Invalid regex pattern '{}': {}", pattern, e),
            ConfigError::InvalidInt(e) => write!(f, "Invalid integer: {}", e),
            ConfigError::ConfigBuild(msg) => write!(f, "Configuration build error: {}", msg),
            ConfigError::ConfigDeserialize(msg) => write!(f, "Configuration deserialization error: {}", msg),
        }
    }
}
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.network.bind_addr, IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)));
