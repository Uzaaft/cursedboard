...
use crate::{ClipboardMessage, decode_message_length, encode_message};
                ...
                match listener.accept() {
                    Ok((stream, addr)) => {
                        ...
                        if should_accept {
                            ...
                            connections.lock().unwrap().insert(addr, ConnectionInfo {
                                addr,
                                is_incoming: true,
                            });
                    ...
                    if should_connect {
                        ...
                        match TcpStream::connect(peer_addr) {
                            Ok(stream) => {
                                ...
                                connections.lock().unwrap().insert(peer_addr, ConnectionInfo {
                                    addr: peer_addr,
                                    is_incoming: false,
                                });
...
pub fn handle_incoming_messages<F>(mut stream: TcpStream, mut message_handler: F) -> std::io::Result<()>
where
...
pub fn send_clipboard_message(stream: &mut TcpStream, msg: &ClipboardMessage) -> std::io::Result<()> {
